INSERT INTO chat.user(name, password)
VALUES ('Maria', 'maria'),
       ('Paul', 'paul'),
       ('Bob', 'bob'),
       ('Mike', 'mike'),
       ('John', 'john') ON CONFLICT (name) DO NOTHING;

INSERT INTO chat.chatroom(name, owner)
VALUES ('room1', (SELECT id FROM chat.user WHERE name = 'Maria')),
       ...
       ('room6', (SELECT id FROM chat.user WHERE name = 'Maria')) ON CONFLICT (name) DO NOTHING;

INSERT INTO chat.message(author, room, text)
VALUES ((SELECT id FROM chat.user WHERE name = 'Maria'), (SELECT id FROM chat.chatroom WHERE name = 'room1'), 'text1'),
       ...
       ((SELECT id FROM chat.user WHERE name = 'Maria'), (SELECT id FROM chat.chatroom WHERE name = 'room6'), 'hi')
       ON CONFLICT DO NOTHING;

INSERT INTO chat.user_chatroom(user_id, chat_id)
VALUES ((SELECT id FROM chat.user WHERE name = 'Maria'), (SELECT id FROM chat.chatroom WHERE name = 'room1')),
       ...
       ((SELECT id FROM chat.user WHERE name = 'Maria'),
        (SELECT id FROM chat.chatroom WHERE name = 'room6')) ON CONFLICT DO NOTHING;